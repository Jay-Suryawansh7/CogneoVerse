...
const IconContainer = memo(({ children, variant, className = '', theme }: { children: React.ReactNode, variant: 'left' | 'center' | 'right', className?: string, theme: string }) => (
  <motion.div
    ...
    className={cn(
      ...
      (!theme || theme === 'light') && "bg-white border border-[var(--color-ink)]/10 group-hover:shadow-xl group-hover:border-[var(--color-oceanic)]/30",
      className
    )}
  >
    <div className={cn(
      ...
      (!theme || theme === 'light') && "text-[var(--color-cobalt)] group-hover:text-[var(--color-oceanic)]"
    )}>
      {children}
    </div>
  </motion.div>
...
const MultiIconDisplay = memo(({ icons, theme }: { icons: React.ReactNode[], theme: string }) => {
...
const Background = ({ theme }: { theme: string }) => (
  <div
    ...
    style={{
      backgroundImage: `radial-gradient(circle at 2px 2px, var(--color-oceanic, #000) 1px, transparent 1px)`,
      backgroundSize: '24px 24px'
    }}
  />
);

interface EmptyStateProps extends Omit<HTMLMotionProps<"section">, "children"> {
    title: string;
    description?: string;
...
export const EmptyState = forwardRef<HTMLElement, EmptyStateProps>(({
  ...
  const getVariantClasses = (variant: string, theme: string) => {
    const variants: Record<string, Record<string, string>> = {
      default: {
        light: "bg-[var(--color-paper)] border-dashed border-2 border-[var(--color-ink)]/20 hover:border-[var(--color-oceanic)]/40 hover:bg-white/50",
  ...
  const getTextClasses = (type: 'title' | 'description', size: string, theme: string) => {
    ...
    const colors: Record<string, Record<string, string>> = {
      title: {
        light: "text-[var(--color-oceanic)]",
        dark: "text-neutral-100",
        neutral: "text-stone-900"
      },
      description: {
        light: "text-[var(--color-ink)]/70",
  ...
  const getButtonClasses = (size: string, theme: string) => {
    ...
    const themeClasses: Record<string, string> = {
      light: "border-[var(--color-ink)]/10 bg-white hover:bg-[var(--color-paper)] text-[var(--color-oceanic)]",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <motion.div variants={CONTENT_VARIANTS} className="space-y-2 mb-6">
            <h2 id={titleId} className={getTextClasses('title', size, theme)} style={{ fontFamily: "var(--font-display)" }}>
              {title}
            </h2>
            {description && (
              <p
