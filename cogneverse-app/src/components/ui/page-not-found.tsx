...
function MessageDisplay() {
  ...
  return (
    <div className="absolute flex flex-col justify-center items-center w-[90%] h-[90%] z-[100]">
      ...
      >
        <div className="text-[35px] font-semibold text-black m-[1%] bg-white/90 px-4 py-1 rounded-lg backdrop-blur-sm">
          Page Not Found
        </div>
        <div className="text-[80px] font-bold text-white m-[1%] drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">
          404
        </div>
        <div className="text-[15px] w-1/2 min-w-[300px] text-center text-white/80 m-[1%] bg-black/50 p-4 rounded-xl backdrop-blur-md border border-white/10">
          The page you are looking for might have been removed, had its name changed, or is temporarily unavailable.
        </div>
        <div className="flex gap-6 mt-8">
          <button
            onClick={() => router.back()}
            className="text-white border-2 border-white/30 hover:bg-white hover:text-black transition-all duration-300 ease-in-out px-6 py-2 h-auto text-base font-medium flex items-center gap-2 hover:scale-105 rounded-full"
          >
            <svg
          ...
          <button
            onClick={() => router.push("/")}
            className="bg-white text-black hover:bg-gray-200 transition-all duration-300 ease-in-out px-6 py-2 h-auto text-base font-medium flex items-center gap-2 hover:scale-105 rounded-full"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
...
function CharactersAnimation() {
  ...
  useEffect(() => {
    ...
    const stickFigures: StickFigure[] = [
      {
        ...
        speedX: 15000, // Slowed down for better effect
      },
      {
        top: '10%',
    ...
    stickFigures.forEach((figure, index) => {
      ...
      stick.style.filter = 'invert(1)'; // Make white for dark background
      ...
      stick.animate(
        [{ left: '100%' }, { left: '-20%' }],
        { duration: figure.speedX, easing: 'linear', fill: 'forwards', iterations: Infinity }
      );
...
function CircleAnimation() {
  ...
  const draw = () => {
    ...
    circulosRef.current.forEach((circulo) => {
      ...
      if (timerRef.current > 65 && timerRef.current < 500) {
       ...
       // Reset circles if they go too far (continuous effect)
      if (circulo.x < -100) {
          circulo.x = canvas.width + Math.random() * 500;
          circulo.size = 1;
      }
    ...
    // Continuous loop instead of stopping
  ...
  return <canvas ref={canvasRef} className="w-full h-full absolute inset-0 mix-blend-screen opacity-30" />;
}
